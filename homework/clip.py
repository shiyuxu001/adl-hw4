...
class CLIP(nn.Module):
    def __init__(
        ...
        # Infer hidden sizes from configs when available
        vision_hidden = getattr(getattr(vision_encoder, "config", None), "hidden_size", None)
        text_hidden = getattr(getattr(text_encoder, "config", None), "hidden_size", None)

        if vision_hidden is None:
            vision_hidden = getattr(getattr(vision_encoder, "config", None), "embed_dim", None)
        if text_hidden is None:
            text_hidden = getattr(getattr(text_encoder, "config", None), "hidden_size", None)

        if vision_hidden is None or text_hidden is None:
            raise ValueError("Could not infer hidden sizes for vision/text encoders")
        ...
        self.logit_scale = nn.Parameter(torch.tensor(float(torch.log(torch.tensor(1.0 / temperature)))))
...
def main():
    ...
    Fire({"train": train, "test": test, "write": write_captions})
